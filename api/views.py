    ...
    def get_queryset(self):
        ...
        queryset = filtered_qs.values("driver_id").annotate(
            ...
            rent_amount=Subquery(
                rent_amount_subquery.filter(driver_id=OuterRef("driver_id")).values(
                    "rent_amount"
                ),
        ...
        total_rent = (
            queryset.aggregate(total_rent=Sum("rent_amount"))["total_rent"] or 0
        )
...
class InvestorCarsEarningsView(CombinedPermissionsMixin, generics.ListAPIView):
...
class CarEfficiencyListView(CombinedPermissionsMixin, generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        filtered_qs = queryset.filter(report_from__range=(start, end)).select_related(
            "vehicle"
        )
        return filtered_qs
    ...
    def list(self, request, *args, **kwargs):
        ...
        aggregated_data = queryset.aggregate(
            total_kasa=Sum("total_kasa"), total_mileage=Sum("mileage")
        )
...
class DriverEfficiencyListView(CombinedPermissionsMixin, generics.ListAPIView):
            ...
            queryset = (
                ...
                .annotate(
                    ...
                    kasa=ExpressionWrapper(
                        Round(
                            Sum("carefficiency__total_kasa") * F("investor_percentage"),
                            2,
                        ),
                    ...
                    progress_percentage=ExpressionWrapper(
                        Case(
                            When(
                                ...
                                then=Round((F("kasa") / F("purchase_price")) * 100),
                            ),
                            default=Value(0),
                    ...
                    progress_percentage=ExpressionWrapper(
                        Case(
                            When(
                                purchase_price__gt=0,
                                then=Round(
                                    ((F("kasa") - F("spending")) / F("purchase_price"))
                                    * 100
                                ),
                            ),
                            default=Value(0),
